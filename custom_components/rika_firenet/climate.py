...
class RikaFirenetStoveClimate(RikaFirenetEntity, ClimateEntity):
    ...
    def __init__(self, config_entry, stove, coordinator):
        ...
        self._attr_translation_key = "stove_climate"
        self._last_mode_change = datetime.now()
        self._mode_change_min_interval = timedelta(seconds=5)
    @property
    ...
    async def async_set_hvac_mode(self, hvac_mode):
        """Set HVAC mode with rate limiting."""
        now = datetime.now()
        ...
        try:
            ...
            if self._stove.get_hvac_mode() != hvac_mode:
                 _LOGGER.warning(f"Mode change to {hvac_mode} not yet applied by Rika cloud")
